...
test('demo map renders fictional grid, incident, units and route', () => {
  ...
  assert.match(source, /candidateRoutes\.map/);
  assert.match(source, /系统推荐/);
  assert.match(source, /data\.congestion\.label/);
  ...
  assert.match(source, /demo-map-route-halo/);
  assert.match(source, /data\.congestion\.routeId/);
  assert.match(source, /data\.congestion\.label/);
