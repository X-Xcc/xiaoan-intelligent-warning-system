...
import { ContactCollisionMap } from '../components/ContactCollisionMap';
...
function SearchScanPreview({ records, scanned, matched }: { records: ContactReviewRecord[]; scanned: number; matched: number }) {
  return <div className="cr-search-scan" aria-label="正在快速扫描图片">
    <div className="cr-search-scan-heading">
      <div><strong>正在快速扫描影像样例</strong><span>图片正在高速刷过，匹配结果即将锁定</span></div>
      <b>{matched} / {scanned.toLocaleString()}</b>
    </div>
...
export function ContactReviewPage({ onBack, onNext, showGait = false }: { onBack: () => void; onNext?: () => void; showGait?: boolean }) {
  ...
  const marked = selectContactRecords(records, { status: '已标记', sort: 'oldest' });
  ...
  return <section className="contact-review-page" aria-label={showGait ? '身份检索工作台' : '视频筛查工作台'}>
    <header className="contact-review-heading">
      <div><div className="ui-eyebrow">影像资料 / 人工复核</div><h1>{showGait ? '身份检索' : '视频筛查'}<span className="cr-heading-ref">{CONTACT_REDACTED_VALUE}</span></h1></div>
      <div className="cr-heading-actions">
        ...
        {!showGait && <button className="ui-button" type="button" disabled={!marked.length} onClick={exportRecords}><Download size={16} />导出复核清单{marked.length > 0 && ` (${marked.length})`}</button>}
      </div>
    </header>
    {!showGait && <div className="cr-source-caption"><ShieldCheck size={14} />AI 合成场景 · 时间、地点与人物关联为预设，非真实证据<span>样例截止 {CONTACT_DEMO_DATE}</span></div>}
    <div className="cr-query-bar">
        <section className="cr-query-section">
          <div className="cr-query-object">{showGait
            ? <div className="cr-query-placeholder" role="img" aria-label="参考对象已脱敏"><ShieldCheck size={20} /><span>已脱敏</span></div>
            : queryImage
            ? <div className="cr-query-placeholder" role="img" aria-label="参考照片已脱敏"><ShieldCheck size={20} /><span>已脱敏</span></div>
            : <div className="cr-query-placeholder" role="img" aria-label="参考照片待上传"><ImageOff size={20} /><span>待上传</span></div>}
            <div><span>参考对象</span><strong>{CONTACT_REDACTED_VALUE}</strong>{!showGait && <button className="ui-text-button cr-upload-button" type="button" onClick={() => uploadRef.current?.click()}><ImagePlus size={14} />{queryImage ? '更换照片' : '上传照片'}</button>}</div></div>
        ...
        <form className="cr-filter-section" onSubmit={(event) => { event.preventDefault(); applyFilters(); }}>
          ...
          {!showGait && <label className="cr-field">
            <span>地点</span>
    ...
    <div className="cr-summary-strip" aria-label="当前结果统计">
      <div><Camera size={18} /><span>出现记录</span><strong>{visibleResultCount}<small> 条</small></strong></div>
      <div><MapPin size={18} /><span>出现点位</span><strong>{visiblePlaceCount}<small> 处</small></strong></div>
      {!showGait && <div><Bookmark size={18} /><span>已标记记录</span><strong>{marked.length}<small> 条</small></strong></div>}
    </div>
    <div className="contact-review-grid">
      <section className="contact-review-results" id="contact-results" aria-label="演示记录">
        <div className="cr-workspace-tabs" role="tablist" aria-label="记录工作区" onKeyDown={event => {
          ...
          const workspaces = ['records', 'map', 'collision'] as const;
          const currentIndex = workspaces.indexOf(workspace);
          const next = event.key === 'Home'
            ? 'records'
            : event.key === 'End'
              ? 'collision'
              : workspaces[(currentIndex + (event.key === 'ArrowRight' ? 1 : -1) + workspaces.length) % workspaces.length];
          setWorkspace(next);
          document.getElementById(next === 'records' ? 'cr-records-tab' : next === 'map' ? 'cr-map-tab' : 'cr-collision-tab')?.focus();
        }}>
          <button type="button" role="tab" id="cr-records-tab" tabIndex={workspace === 'records' ? 0 : -1} aria-controls="cr-workspace-panel" aria-selected={workspace === 'records'} onClick={() => setWorkspace('records')}><LayoutGrid size={17} />抓拍记录<span>{visibleResultCount}</span></button>
          {!showGait && <button type="button" role="tab" id="cr-map-tab" tabIndex={workspace === 'map' ? 0 : -1} aria-controls="cr-workspace-panel" aria-selected={workspace === 'map'} onClick={() => setWorkspace('map')}><MapIcon size={17} />出现点位<span>{visiblePlaceCount}</span></button>}
          {!showGait && <button type="button" role="tab" id="cr-collision-tab" tabIndex={workspace === 'collision' ? 0 : -1} aria-controls="cr-workspace-panel" aria-selected={workspace === 'collision'} onClick={() => setWorkspace('collision')}><MapPin size={17} />轨迹碰撞<span>68%</span></button>}
        </div>
        {!showGait && <div className="cr-status-tabs" role="group" aria-label="记录状态">{statuses.map((status) => <button type="button" key={status} className={filters.status === status ? 'active' : ''} aria-pressed={filters.status === status} onClick={() => setFilters((current) => ({ ...current, status }))}>{status === 'all' ? '全部记录' : status}<span>{searchPhase === 'idle' ? 0 : status === 'all' ? scopeRecords.length : scopeRecords.filter((record) => record.status === status).length}</span></button>)}</div>}
        <div className="cr-results-controls">
          ...
          {!showGait && <><select aria-label="结果排序" value={filters.sort} onChange={(event) => setFilters((current) => ({ ...current, sort: event.target.value as ContactFilters['sort'] }))}><option value="newest">时间倒序</option><option value="oldest">时间正序</option><option value="frequency">角色出现次数</option></select>
          <select aria-label="角色筛选" value={filters.companion ?? ''} onChange={(event) => setFilters((current) => ({ ...current, companion: event.target.value }))}><option value="">全部角色</option>{companions.map((person) => <option key={person.id} value={person.id}>{contactCompanionDisplayLabel(person)}</option>)}</select>
        ...
        </div> : workspace !== 'collision' && searchPhase === 'complete' && workspace === 'map' ? <div className="cr-map-workspace">
