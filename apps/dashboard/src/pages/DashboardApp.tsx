...
import { NanchangAmapMap } from '../components/NanchangAmapMap';
...
const pathView = (): View => (window.location.pathname.startsWith('/command') ? 'command' : window.location.pathname.startsWith('/admin') ? 'admin' : 'entry');
...
export function DashboardApp() {
  ...
  const navigate = (next: View) => {
    window.history.pushState({}, '', next === 'entry' ? '/' : `/${next}`);
    setView(next);
    setMobileOpen(false);
  ...
  return (
    <main className="app-shell">
        ...
        <>
          <header className="topbar-wrap">
            <nav className="glass-nav">
              <button className="brand-lockup" onClick={() => navigate('entry')}>
              ...
              <div className="nav-actions">
                {apiOnline && (
                  <span className="sync-chip online">
                    <CircleDot size={12} />
                    FastAPI 在线
                  </span>
                )}
                <button className="icon-button mobile-menu" onClick={() => setMobileOpen(!mobileOpen)} aria-label="打开导航">
      ...
      <div className="page-frame">
        {view === 'entry' && <EntryPage apiOnline={apiOnline} navigate={navigate} />}
        {view === 'command' && <CommandPage overview={overview} refreshing={refreshing} refresh={loadOverview} notify={notify} />}
        {view === 'admin' && <AdminPage overview={overview} apiOnline={apiOnline} refresh={loadOverview} />}
      </div>
  ...
  return (
    <section className="workspace-page">
      <div className="workspace-heading">
        <div>
          <span className="section-kicker">Command Frontend / Live Operations</span>
          <h1>夜市智防指挥前端</h1>
          <p>面向指挥人员和巡防队伍的实时事件处置工作台。</p>
        </div>
      ...
      <div className="metric-grid">
        ...
        <Metric icon={<Clock3 size={18} />} label="待处置" value={overview.stats.pending_orders} detail="按优先级排队" tone="orange" />
        <Metric icon={<Users size={18} />} label="在线巡防" value={overview.stats.online_staff} detail="4 个值守角色" tone="blue" />
      ...
      <div className="command-grid">
        <section className="glass-panel queue-panel">
          ...
          <div className="event-list">
            {filtered.map((event) => (
              <button className={`event-list-item ${selected?.id === event.id ? 'selected' : ''}`} key={event.id} onClick={() => setSelectedId(event.id)}>
        ...
        <section className="glass-panel map-panel">
          <PanelTitle kicker="MAP / GRID" title="南昌各夜市实时态势" icon={<span className="map-live"><span />{nightMarkets.length} 个夜市</span>} />
          <div className="market-tabs">
            {nightMarkets.slice(0, 8).map((market) => (
              <button className={selectedMarket?.id === market.id ? 'active' : ''} key={market.id} onClick={() => setSelectedMarketId(market.id)}>
                {market.name}
              </button>
            ))}
          </div>
          <div className="map-stage">
            <NanchangAmapMap markets={nightMarkets} selectedMarketId={selectedMarket?.id ?? nightMarkets[0]?.id ?? ''} />
          </div>
          <div className="map-action-row">
            <button className="quiet-button" onClick={() => openNavigation(selectedMarket)} disabled={!selectedMarket}>
              高德导航到此夜市 <ArrowRight size={15} />
            </button>
            <div className="map-footnote">数据源：后端 /api/events/overview · 夜市坐标来自项目夜市清单 · 底图由高德地图 JS API 提供</div>
          </div>
        </section>
