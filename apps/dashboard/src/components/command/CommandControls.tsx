...
export function CommandControls({ value, onChange, disabled = false }: {
  value: ControlState; onChange: (state: ControlState) => void; disabled?: boolean;
}) {
  return <div className="command-controls" data-command-controls tabIndex={0} aria-label="演示控屏画布"
    ...
    <nav aria-label="屏台阶段">{stages.map((stage, index) =>
      <button type="button" key={stage} aria-pressed={value.stage === stage} disabled={disabled}
        onClick={() => onChange({ ...value, stage, reveal: 0, revision: value.revision + 1 })}>
        {index < 4 ? `B${index + 1}` : '移交'}
      </button>)}</nav>
    <div className="command-control-tools">
      <button type="button" aria-label="上一步" title="上一步" disabled={disabled} onClick={() => onChange(advanceControl(value, 'previous'))}><ArrowLeft size={18} /></button>
      <button type="button" aria-label={value.paused ? '继续' : '暂停'} title={value.paused ? '继续' : '暂停'} disabled={disabled}
        onClick={() => onChange(advanceControl(value, 'pause'))}>{value.paused ? <Play size={18} /> : <Pause size={18} />}</button>
      <button type="button" aria-label="下一步" title="下一步" disabled={disabled} onClick={() => onChange(advanceControl(value, 'next'))}><ArrowRight size={18} /></button>
      <button type="button" aria-label="复位画面" title="复位画面" disabled={disabled} onClick={() => onChange(advanceControl(value, 'reset'))}><RotateCcw size={18} /></button>
      <button type="button" aria-label="教学备用页" title="教学备用页" disabled={disabled} onClick={() => onChange({
