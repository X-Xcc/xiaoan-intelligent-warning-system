...
def validate_host(value: str) -> str:
    ...
    if not value.isascii() or not _clean(value) or any(c.isspace() for c in value):
        raise ValueError(error)
    host = value.lower()
...
def _validate_path(path: str) -> None:
    ...
    for _ in range(5):
        ...
        if parts.netloc or parts.scheme or any(
            re.sub(r"[^a-z]", "", key.lower()) in sensitive for key, _ in pairs
        ):
            raise ValueError(error)
        try:
...
def validate_config(data: dict, previous: dict | None = None) -> dict:
    ...
    result = {**DEFAULTS, **(previous or {}), **data}
    for key, limit in (("name", 120), ("host", 253), ("username", 256),
                       ("password", 512), ("rtspPath", 2048)):
    ...
    for key, allowed in (("kind", ("go2", "hikvision", "dahua", "rtsp")),
